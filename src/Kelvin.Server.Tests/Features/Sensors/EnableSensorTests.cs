...
public class EnableSensorTests
{
    [Fact]
    public async Task EnablesSensorClearsCacheAndNotifies()
    {
        using var harness = new KelvinContextHarness();
        await using var seedContext = harness.CreateContext();
        var sensor = new Sensor { Enabled = false };
        ...
        var result = await new EnableSensorHandler(context, cache, hub).HandleAsync(
            new EnableSensorRequest(sensorId)
        );
        ...
        (await harness.CreateContext().Sensors.FindAsync(sensorId))!.Enabled.ShouldBeTrue();
    }
}
